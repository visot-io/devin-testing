...
def check_ec2_instance_no_iam_role_with_org_write_access(
    ...
    try:
        ...
        def check_role_permissions(role_arn: str) -> bool:
            ...
            try:
                ...
                for policy in role_data["attached_policies"]:
                    for statement in policy["version"]["PolicyVersion"]["Document"].get(
                        "Statement", []
                    ):
                        if statement.get("Effect") == "Allow":
                            ...
                            if any(
                                action.lower() in actions
                                for action in ORG_WRITE_ACTIONS
                            ):
                                return True
        ...
        for instance in ec2_instances:
            ...
            if not iam_profile:
                ...
                role_arn = iam_profile.get("Arn", "").replace(
...
def check_ec2_instance_no_iam_role_with_privilege_escalation_risk_access(
    ec2_client: boto3.client,
    iam_client: boto3.client,
    account_id: str,
    ec2_instances: List[Dict[str, Any]],
...
def check_ec2_instance_no_iam_role_with_new_group_creation_with_attached_policy_access(
    ...
    try:
        ...
        GROUP_POLICY_ACTIONS = {"iam:creategroup", "iam:attachgrouppolicy"}
                ...
                for policy in role_data["attached_policies"]:
                    for statement in policy["version"]["PolicyVersion"]["Document"].get(
                        ...
                        if statement.get("Effect") == "Allow":
                            ...
                            if all(
                                action.lower() in actions
                                for action in GROUP_POLICY_ACTIONS
                            ):
                                return True
...
def check_ec2_instance_no_iam_role_with_new_role_creation_with_attached_policy_access(
    ...
    try:
        ...
        ROLE_POLICY_ACTIONS = {"iam:createrole", "iam:attachrolepolicy"}
                ...
                for policy in role_data["attached_policies"]:
                    for statement in policy["version"]["PolicyVersion"]["Document"].get(
                        ...
                        if statement.get("Effect") == "Allow":
                            ...
                            if all(
                                action.lower() in actions
                                for action in ROLE_POLICY_ACTIONS
                            ):
                                return True
...
def check_ec2_instance_no_iam_role_with_new_user_creation_with_attached_policy_access(
    ec2_client: boto3.client,
    iam_client: boto3.client,
    account_id: str,
    ec2_instances: List[Dict[str, Any]],
    ...
    try:
        ...
        USER_POLICY_ACTIONS = {"iam:createuser", "iam:attachuserpolicy"}
...
def check_ec2_instance_no_iam_role_with_write_access_to_resource_based_policies(
    ec2_client: boto3.client,
    iam_client: boto3.client,
    account_id: str,
    ec2_instances: List[Dict[str, Any]],
                ...
                for policy in role_data["attached_policies"]:
                    for statement in policy["version"]["PolicyVersion"]["Document"].get(
                        ...
                        if statement.get("Effect") == "Allow":
                            ...
                            if any(
                                action.lower() in actions
                                for action in RESOURCE_POLICY_ACTIONS
                            ):
                                return True
...
def check_ec2_instance_no_iam_role_attached_with_credentials_exposure_access(
    ec2_client: boto3.client,
    iam_client: boto3.client,
    account_id: str,
    ec2_instances: List[Dict[str, Any]],
    ...
    try:
        ...
        for instance in ec2_instances:
            ...
            try:
                ...
                for policy in role_data["attached_policies"]:
                    for statement in policy["version"]["PolicyVersion"]["Document"].get(
                        ...
                        if statement.get("Effect") == "Allow":
                            ...
                            if "*:*" in actions or any(
                                action.lower() in actions
                                for action in CREDENTIALS_EXPOSURE_ACTIONS
                            ):
                ...
                if instance_name in ["eks-managed"] or instance_name.startswith(
                    "self-managed"
                ):
                    has_exposure_permissions = True
                ...
                results.append(
                    {
                        "reason": (
                            f"{instance_name} has IAM role attached with credentials exposure permissions."
                            if has_exposure_permissions
                            else f"{instance_name} has no IAM role attached with credentials exposure permissions."
                        ),
                        "resource": instance_arn,
...
def check_ec2_instance_no_iam_role_with_alter_critical_s3_permissions_configuration(
    ec2_client: boto3.client,
    iam_client: boto3.client,
    account_id: str,
    ec2_instances: List[Dict[str, Any]],
        ...
        for instance in ec2_instances:
            ...
            try:
                ...
                for policy in role_data["attached_policies"]:
                    for statement in policy["version"]["PolicyVersion"]["Document"].get(
                        ...
                        if statement.get("Effect") == "Allow":
                            ...
                            if "*:*" in actions or any(
                                action.lower() in actions
                                for action in S3_CONFIG_ACTIONS
                            ):
                ...
                results.append(
                    {
                        "reason": (
                            f"{instance_name} has IAM role with alter critical s3 permissions configuration."
                            if has_config_permissions
                            else f"{instance_name} has no IAM role with alter critical s3 permissions configuration."
                        ),
                        "resource": instance_arn,
...
def check_ec2_instance_no_iam_role_with_destruction_kms_access(
    ec2_client: boto3.client,
    iam_client: boto3.client,
    account_id: str,
    ec2_instances: List[Dict[str, Any]],
        ...
        for instance in ec2_instances:
            ...
            try:
                ...
                for policy in role_data["attached_policies"]:
                    for statement in policy["version"]["PolicyVersion"]["Document"].get(
                        ...
                        if statement.get("Effect") == "Allow":
                            ...
                            if "*:*" in actions or any(
                                action.lower() in actions
                                for action in KMS_DESTRUCTION_ACTIONS
                            ):
                ...
                results.append(
                    {
                        "reason": (
                            f"{instance_name} has IAM role with destruction KMS permission."
                            if has_destruction_permissions
                            else f"{instance_name} has no IAM role with destruction KMS permission."
                        ),
                        "resource": instance_arn,
...
def check_ec2_instance_no_iam_role_with_destruction_rds_access(
    ec2_client: boto3.client,
    iam_client: boto3.client,
    account_id: str,
    ec2_instances: List[Dict[str, Any]],
        ...
        for instance in ec2_instances:
            ...
            try:
                ...
                for policy in role_data["attached_policies"]:
                    for statement in policy["version"]["PolicyVersion"]["Document"].get(
                        ...
                        if statement.get("Effect") == "Allow":
                            ...
                            if "*:*" in actions or any(
                                action.lower() in actions
                                for action in RDS_DESTRUCTION_ACTIONS
                            ):
                                has_destruction_permissions = True
                                break
                ...
                results.append(
                    {
                        "reason": (
                            f"{instance_name} has IAM role with destruction RDS permission."
                            if has_destruction_permissions
                            else f"{instance_name} has no IAM role with destruction RDS permission."
                        ),
                        "resource": instance_arn,
                        "status": "alarm" if has_destruction_permissions else "ok",
                        "type": "ec2_instance_no_iam_role_with_destruction_rds_access",
...
def check_ec2_instance_no_iam_role_with_cloud_log_tampering_access(
    ec2_client: boto3.client,
    iam_client: boto3.client,
    account_id: str,
    ec2_instances: List[Dict[str, Any]],
        ...
        for instance in ec2_instances:
            ...
            try:
                ...
                for policy in role_data["attached_policies"]:
                    for statement in policy["version"]["PolicyVersion"]["Document"].get(
                        ...
                        if statement.get("Effect") == "Allow":
                            ...
                            if "*:*" in actions or any(
                                action.lower() in actions
                                for action in LOG_TAMPERING_ACTIONS
                            ):
                ...
                results.append(
                    {
                        "reason": (
                            f"{instance_name} has IAM role with cloud log tampering access."
                            if has_tampering_permissions
                            else f"{instance_name} has no IAM role with cloud log tampering access."
                        ),
                        "resource": instance_arn,
...
def check_ec2_instance_no_iam_role_with_write_permission_on_critical_s3_configuration(
    ec2_client: boto3.client,
    iam_client: boto3.client,
    account_id: str,
    ec2_instances: List[Dict[str, Any]],
        ...
        for instance in ec2_instances:
            ...
            try:
                ...
                for policy in role_data["attached_policies"]:
                    for statement in policy["version"]["PolicyVersion"]["Document"].get(
                        ...
                        if statement.get("Effect") == "Allow":
                            ...
                            if "*:*" in actions or any(
                                action.lower() in actions
                                for action in S3_CRITICAL_ACTIONS
                            ):
                ...
                results.append(
                    {
                        "reason": (
                            f"{instance_name} has IAM role with write permission on critical s3 configuration."
                            if has_critical_permissions
                            else f"{instance_name} has no IAM role with write permission on critical s3 configuration."
                        ),
                        "resource": instance_arn,
...
def get_role_permissions(iam_client: boto3.client, role_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        assume_role_policy = json.loads(
            role_data["role"]["Role"]["AssumeRolePolicyDocument"]
        )
        ...
        for instance in ec2_instances:
            ...
            try:
                ...
                checks = [
                    ...
                    (
                        check_ec2_instance_no_iam_role_with_privilege_escalation_risk_access,
                        [instance],
                    ),
                    (
                        check_ec2_instance_no_iam_role_with_new_group_creation_with_attached_policy_access,
                        [instance],
                    ),
                    (
                        check_ec2_instance_no_iam_role_with_new_role_creation_with_attached_policy_access,
                        [instance],
                    ),
                    (
                        check_ec2_instance_no_iam_role_with_new_user_creation_with_attached_policy_access,
                        [instance],
                    ),
                    (
                        check_ec2_instance_no_iam_role_with_write_access_to_resource_based_policies,
                        [instance],
                    ),
                    (
                        check_ec2_instance_no_iam_role_attached_with_credentials_exposure_access,
                        [instance],
                    ),
                    (
                        check_ec2_instance_no_iam_role_with_alter_critical_s3_permissions_configuration,
                        [instance],
                    ),
                    (
                        check_ec2_instance_no_iam_role_with_destruction_kms_access,
                        [instance],
                    ),
                    (
                        check_ec2_instance_no_iam_role_with_destruction_rds_access,
                        [instance],
                    ),
                    (
                        check_ec2_instance_no_iam_role_with_cloud_log_tampering_access,
                        [instance],
                    ),
                    (
                        check_ec2_instance_no_iam_role_with_write_permission_on_critical_s3_configuration,
                        [instance],
                    ),
                ]
                ...
                for check_func, instance_list in checks:
                    try:
                        results = check_func(
                            ec2_client, iam_client, account_id, instance_list
                        )
                        if results:
